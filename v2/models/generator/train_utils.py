"""
models/generator/train_utils.py — v2 (Research-Grade)
======================================================
...
Features:
  ...
  - R-Drop regularization (two forward passes, KL divergence)
...
def quality_filter(example, min_len=50, max_len=10000):
    ...
    Checks:
      - Minimum text length (avoids trivially short examples)
      - Maximum text length (avoids extremely long examples)
      - Minimum entropy (avoids repetitive/degenerate text)
      - Not too many blank lines (avoids formatting artifacts)
    """
...
def get_wsd_schedule(optimizer, warmup_steps, stable_steps, decay_steps, peak_lr, min_lr_ratio=0.1):
    ...
    WSD is superior to cosine because it doesn't require pre-committing to
    total steps, allows checkpoint branching, and produces sharper final
    convergence (MiniCPM, 2024).
    """
...
def train_epoch(model, dataloader, optimizer, scheduler, device,
    ...
    Returns (avg_loss, num_batches, global_step).
    """
    model.train()
    total_loss = 0.0
    num_batches = 0
...
def make_training_configs(env):
    """Build hardware-aware training configs (v2 research-grade).

    Research-grade defaults:
      - WSD schedule: warmup_pct, stable_pct, decay_pct
      - EMA decay for weight averaging
      - Label smoothing for regularization
      - Longer epochs with more patience
    ...
    return {
        "pretrain": {
            ...
            "patience": 3, "ckpt_every": 2000, "lr_finder": True,
        ...
        "domain": {
            ...
            "betas": (0.9, 0.95), "epochs": 4, "batch_size": batch,
            "grad_clip": 1.0, "max_len": 1024, "grad_accum_steps": accum,
            "patience": 3, "ckpt_every": 2000, "lr_finder": True,
            "ema_decay": 0.999, "label_smoothing": 0.05,
            "schedule": "wsd", "stable_pct": 0.80, "decay_pct": 0.18,
        ...
        "interview": {
            ...
            "patience": 3, "ckpt_every": 1000, "lr_finder": True,
            "ema_decay": 0.999, "label_smoothing": 0.05,
            "schedule": "wsd", "stable_pct": 0.80, "decay_pct": 0.18,
